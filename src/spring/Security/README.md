...
#### 54.
To use a database as storage for your user and role credentials you have to set up two tables: `users` and `authorities` in the `users` table you save the username as primary key, the password in the following scheme `{HASH_ALORITHM}PASSWORD` and a bool enabled. In the `authorities`  table you stare the username of the users table as foreign key and the authority in the scheme `ROLE_YOURROLE`. An example db scrip is at the end of the README.
After setting up the database and inserting the users and roles you have to add a new bean to the DemoAppConfig that returns the DataSource to the MySql connection. In the DemoSpringConfig you have to change the hardcoded users and roles to a new method `auth.jdbcAuthentication().dataSource(securityDataSource);` where auth is the Autowired new bean created in the DemoAppConfig. 
